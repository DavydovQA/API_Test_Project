...
class Checking():
    ...
    @staticmethod
    def check_json_value(response: Response, field_name, expected_value):  # field_name - то поле по которому проверяем
        check = response.json()                                            # expected_value - значение этого поля
